...
struct LiFXHeader {
    ...
    init(data: NSData) {
        var byte16 = UInt16(0)
        data.getBytes(&byte16, length: 2)
        size = byte16
        
        data.getBytes(&byte16, range: NSMakeRange(2, 2))
        prtcl = byte16 & 0xFFF
        ...
        var byte32 = UInt32(0)
        data.getBytes(&byte32, range: NSMakeRange(4, 4))
        source = byte32
        
        var targetBuffer = [UInt8](count: 8, repeatedValue: 0)
        data.getBytes(&targetBuffer, range: NSMakeRange(8, 8))
        ...
        var byte8 = UInt8(0)
        data.getBytes(&byte8, range: NSMakeRange(22, 1))
        ack_required = (byte8 >> 1) & 0x1 == 1
        res_required = byte8 & 0x1 == 1
        
        data.getBytes(&byte8, range: NSMakeRange(23, 1))
        sequence = byte8
        
        data.getBytes(&byte16, range: NSMakeRange(32, 2))
        type = byte16
    }
